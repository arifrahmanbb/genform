...
$genform_f  = $genform_id ? $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}genform_forms WHERE id = %d", $genform_id ) ) : null;
$genform_s  = $genform_f ? json_decode( $genform_f->form_settings, true ) : array();
?>
	...
	<form method="post" id="gfm-builder-form">
		...
		<div class="gfm-builder-top">
			<div class="gfm-card gfm-name-card">
				...
				<input type="text" name="form_name" value="<?php echo esc_attr( $genform_f->form_name ?? '' ); ?>" required placeholder="<?php esc_attr_e( 'e.g. Contact Us', 'genform' ); ?>">
			</div>
				...
				<div class="gfm-sidebar">
					<div class="gfm-card">
						...
						<div class="gfm-field-buttons">
							...
							foreach ( $genform_fields as $genform_t => $genform_i ) :
								?>
								<button type="button" class="gfm-add-field" data-type="<?php echo esc_attr( $genform_t ); ?>">
									<span class="dashicons dashicons-<?php echo esc_attr( $genform_i ); ?>"></span>
									<?php echo esc_html( ucfirst( $genform_t ) ); ?>
								</button>
							<?php endforeach; ?>
						</div>
					</div>
				...
				<div class="gfm-grid">
					<div class="gfm-col">
						...
						<select id="gfm-con-type">
							<option value="message" <?php selected( $genform_s['con_type'] ?? '', 'message' ); ?>><?php esc_html_e( 'Message', 'genform' ); ?></option>
							<option value="redirect" <?php selected( $genform_s['con_type'] ?? '', 'redirect' ); ?>><?php esc_html_e( 'Redirect', 'genform' ); ?></option>
						</select>
					</div>
					<div class="gfm-col gfm-con-field gfm-con-redirect gfm-hidden">
						<label><?php esc_html_e( 'URL', 'genform' ); ?></label>
						<input type="url" id="gfm-redirect-url" value="<?php echo esc_url( $genform_s['redirect_url'] ?? '' ); ?>">
					</div>
				</div>
				<div class="gfm-grid gfm-con-field gfm-con-message">
					<div class="gfm-col">
		<label><?php esc_html_e( 'Success Message', 'genform' ); ?></label>
						<textarea id="gfm-success-message"><?php echo esc_textarea( $genform_s['success_message'] ?? '' ); ?></textarea>
					</div>
					<div class="gfm-col">
						<label><?php esc_html_e( 'Error Message', 'genform' ); ?></label>
						<textarea id="gfm-error-message"><?php echo esc_textarea( $genform_s['error_message'] ?? '' ); ?></textarea>
					</div>
				</div>
			</div>
			...
			<div class="gfm-card">
				...
				<div class="gfm-grid">
					<div class="gfm-col">
						...
						<input type="text" id="gfm-submit-text" value="<?php echo esc_attr( $genform_s['submit_text'] ?? esc_html__( 'Submit', 'genform' ) ); ?>">
					</div>
					<div class="gfm-col">
						<label><?php esc_html_e( 'Button Alignment', 'genform' ); ?></label>
						<select id="gfm-submit-align">
							<option value="left" <?php selected( $genform_s['submit_align'] ?? 'left', 'left' ); ?>><?php esc_html_e( 'Left', 'genform' ); ?></option>
							<option value="center" <?php selected( $genform_s['submit_align'] ?? 'left', 'center' ); ?>><?php esc_html_e( 'Center', 'genform' ); ?></option>
							<option value="right" <?php selected( $genform_s['submit_align'] ?? 'left', 'right' ); ?>><?php esc_html_e( 'Right', 'genform' ); ?></option>
							<option value="full" <?php selected( $genform_s['submit_align'] ?? 'left', 'full' ); ?>><?php esc_html_e( 'Full Width', 'genform' ); ?></option>
						</select>
					</div>
				</div>
			</div>
			...
			<div class="gfm-card">
				...
				<div class="gfm-grid">
					<div class="gfm-col">
						...
						<input type="number" id="gfm-base-font-size" value="<?php echo esc_attr( $genform_s['base_font_size'] ?? '16' ); ?>" min="12" max="24">
					</div>
					<div class="gfm-col">
						<label><?php esc_html_e( 'Font Weight', 'genform' ); ?></label>
						<select id="gfm-base-font-weight">
							<option value="300" <?php selected( $genform_s['base_font_weight'] ?? '400', '300' ); ?>><?php esc_html_e( 'Light (300)', 'genform' ); ?></option>
							<option value="400" <?php selected( $genform_s['base_font_weight'] ?? '400', '400' ); ?>><?php esc_html_e( 'Regular (400)', 'genform' ); ?></option>
							<option value="500" <?php selected( $genform_s['base_font_weight'] ?? '400', '500' ); ?>><?php esc_html_e( 'Medium (500)', 'genform' ); ?></option>
							<option value="600" <?php selected( $genform_s['base_font_weight'] ?? '400', '600' ); ?>><?php esc_html_e( 'Semi-Bold (600)', 'genform' ); ?></option>
							<option value="700" <?php selected( $genform_s['base_font_weight'] ?? '400', '700' ); ?>><?php esc_html_e( 'Bold (700)', 'genform' ); ?></option>
						</select>
					</div>
				</div>
			</div>
			...
			<div class="gfm-card">
				...
				<div class="gfm-grid">
					<div class="gfm-col">
						...
						<input type="text" id="gfm-admin-email" value="<?php echo esc_attr( $genform_s['admin_email'] ?? '{admin_email}' ); ?>">
					...
					<div class="gfm-col">
						...
						<input type="text" id="gfm-from-name" value="<?php echo esc_attr( $genform_s['from_name'] ?? '' ); ?>" placeholder="{global_from_name}">
					</div>
				</div>
				<div class="gfm-grid">
					<div class="gfm-col">
						<label><?php esc_html_e( 'From Email', 'genform' ); ?></label>
						<input type="text" id="gfm-from-email" value="<?php echo esc_attr( $genform_s['from_email'] ?? '' ); ?>" placeholder="{global_from_email}">
					</div>
					<div class="gfm-col">
						<label><?php esc_html_e( 'Reply-To Email', 'genform' ); ?></label>
						<input type="text" id="gfm-reply-to" value="<?php echo esc_attr( $genform_s['reply_to'] ?? '{field_email}' ); ?>">
					</div>
				</div>
				<div class="gfm-setting-row gfm-mt-md">
					<label><?php esc_html_e( 'Email Subject', 'genform' ); ?></label>
					<input type="text" id="gfm-email-subject" value="<?php echo esc_attr( $genform_s['email_subject'] ?? '' ); ?>" class="widefat" placeholder="<?php esc_attr_e( 'New Submission: {form_name}', 'genform' ); ?>">
				</div>
				<div class="gfm-setting-row gfm-mt-md">
					<label><?php esc_html_e( 'Email Body', 'genform' ); ?></label>
					<textarea id="gfm-email-body" rows="8" class="widefat" placeholder="<?php esc_attr_e( "{all_fields}\n\nSent from {site_title}", 'genform' ); ?>"><?php echo esc_textarea( $genform_s['email_body'] ?? '' ); ?></textarea>
