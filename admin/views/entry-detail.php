...
$genform_entry = $wpdb->get_row($wpdb->prepare(
...
if ('unread' === $genform_entry->status) {
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $wpdb->update("{$wpdb->prefix}genform_entries", ['status' => 'read'], ['id' => $genform_entry_id]);
}

$genform_entry_data     = json_decode($genform_entry->entry_data, true) ?: [];
$genform_entry_metadata = json_decode($genform_entry->entry_metadata, true) ?: [];
...
if (empty($genform_resolved)) {
    foreach ($genform_entry_data as $genform_key => $genform_value) {
        $genform_resolved[ucwords(str_replace(['_', '-'], ' ', $genform_key))] = $genform_value;
    }
}
?>
...
<div class="wrap genform-admin-wrap">
    ...
    <div class="gfm-entry-detail-layout" style="display: grid; grid-template-columns: 1fr 320px; gap: 20px;">
        <!-- Main Content -->
        <div class="gfm-card">
        ...
        <!-- Sidebar -->
        <div>
            <div class="gfm-card" style="margin-bottom: 16px;">
                <h3><?php esc_html_e('Entry Info', 'genform'); ?></h3>
                <div class="gfm-entry-meta" style="line-height: 2;">
                    <p>
