...
final class Manager
{
    ...
    {
        add_action('wp_ajax_genform_create_from_template', [self::class, 'ajaxCreateFromTemplate']);
    }
    ...
    public static function ajaxCreateFromTemplate(): void
    {
        ...
        $fields_raw = isset($_POST['template_fields'])
            ? wp_unslash($_POST['template_fields'])
            : '';

        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $settings_raw = isset($_POST['template_settings'])
            ? wp_unslash($_POST['template_settings'])
            : '';

        if (! $form_name || ! $fields_raw) {
            wp_send_json_error(['message' => esc_html__('Invalid template data.', 'genform')]);
        }
        ...
        if (! is_array($fields)) {
            wp_send_json_error(['message' => esc_html__('Invalid field configuration.', 'genform')]);
        }
        ...
        $inserted = $wpdb->insert(
            ...
            [
                'form_name'     => $form_name,
                'form_data'     => wp_json_encode(['fields' => $normalized]),
        ...
        if (! $inserted) {
            wp_send_json_error(['message' => esc_html__('Failed to create form.', 'genform')]);
        }
