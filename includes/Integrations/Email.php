...
final class Email
{
	...
	public static function send(int $entry_id, int $form_id, array $data): void
	{
		...
		$from_name  = str_replace(array_keys($tags), array_values($tags), ! empty($settings['from_name']) ? $settings['from_name'] : ($global_options['from_name'] ?? get_bloginfo('name')));
		$from_email = str_replace(array_keys($tags), array_values($tags), ! empty($settings['from_email']) ? $settings['from_email'] : ($global_options['from_email'] ?? get_bloginfo('admin_email')));
