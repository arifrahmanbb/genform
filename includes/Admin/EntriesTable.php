...
class EntriesTable extends \WP_List_Table
{
	...
	public function prepare_items()
	{
		...
		$allowed_orderby = ['id', 'created_at', 'form_name'];
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function get_bulk_actions()
	{
		...
		return ['trash' => esc_html__('Move to Trash', 'genform')];
	}
	...
	public function column_actions($item)
	{
		...
		if (empty($resolved_data)) {
			foreach ($entry_data as $key => $value) {
				$resolved_data[ucwords(str_replace(['_', '-'], ' ', $key))] = $value;
			}
		}
	...
	public function extra_tablenav($which)
	{
		...
		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$available_forms  = $wpdb->get_results("SELECT id, form_name FROM {$wpdb->prefix}genform_forms ORDER BY form_name ASC");
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
